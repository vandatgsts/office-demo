...
public class SlideDrawKit {
    ...
    private void drawTable(Canvas canvas, PGModel pgModel, PGEditor editor, int slideNo, TableShape table, float zoom, Map<Integer, Map<Integer, IAnimation>> shapeVisible) {
        ...
        if (table.getAnimation() != null
            ...
            if (tableRect != null) {
                // Sử dụng saveLayerAlpha mà không có các cờ
                canvas.saveLayerAlpha(new RectF(
                                tableRect.x * zoom,
                                tableRect.y * zoom,
                                (tableRect.x + tableRect.width + 1) * zoom,
                                (tableRect.height + tableRect.y + 1) * zoom),
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (cell != null) {
                ...
                Rect brRect = new Rect(
                        Math.round(rect.getX() * zoom),
                        Math.round(rect.getY() * zoom),
                        Math.round((rect.getX() + rect.getWidth()) * zoom),
                        Math.round((rect.getY() + rect.getHeight()) * zoom));
                // background
