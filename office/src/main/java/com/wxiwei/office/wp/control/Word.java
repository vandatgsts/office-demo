...
//            addView(printWord);
    ...
    private void toPicture(IOfficeToPicture otp) {
        ...
        if (bitmap.getWidth() != getWidth() || bitmap.getHeight() != getHeight()) {
            float newZoom = Math.min((float) bitmap.getWidth() / getWidth(), (float) bitmap.getHeight() / getHeight()) * getZoom();
    ...
    public void layoutPrintMode() {
        post(() -> {
            if (currentRootType == WPViewConstant.PRINT_ROOT && printWord != null) {
...
//                addView(printWord);
    ...
    private void scrollToFocusXY(float newScale, float oldScale, int focusScreenX, int focusScreenY) {
        ...
        if (getCurrentRootType() == WPViewConstant.PAGE_ROOT && pageRoot != null && pageRoot.getChildView() != null) {
        ...
        scrollBy((int) ((pageWidth - lastpageWidth) * ratioX), (int) ((pageHeight - lastpageHeight) * ratioY));
    }
    ...
    public Rectangle getPageSize(int pageIndex) {
        ...
        PageView pv = WPViewKit.instance().getPageView(pageRoot, (int) (getScrollX() / zoom), (int) (getScrollY() / zoom) + getHeight() / 5);
        if (pv == null) {
    ...
    protected void showPage(int index, int direction) {
        if (index < 0 || index >= getPageCount() || getCurrentRootType() == WPViewConstant.NORMAL_ROOT) {
            return;
        }
        if (getCurrentRootType() == WPViewConstant.PRINT_ROOT) {
    ...
    public Bitmap pageToImage(int pageNumber) {
        if (pageNumber <= 0 || pageNumber > getPageCount() || pageRoot == null || pageRoot.getChildView() == null || getCurrentRootType() == WPViewConstant.NORMAL_ROOT) {
            return null;
        }
